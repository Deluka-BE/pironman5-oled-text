import sys, types, unittest, importlib.util, logging
...
class Screen:
    def __init__(self, **kwargs):
        self.font_8 = types.SimpleNamespace(getlength=lambda s: len(s)*6)
        self.frames = []; self.buffer = []
    def is_ready(self): return True
    def clear(self): self.buffer = []
    def display(self): self.frames.append(list(self.buffer))
    def draw_text(self, text, x, y): self.buffer.append((text, x, y))
...
class Tests(unittest.TestCase):
    ...
    def test_expiry_returns_to_normal(self):
        ...
        self.assertEqual(self.oled.oled.frames[-1][0][0], 'Hello')
    ...
    def test_persistent_replace_clear(self):
        ...
        self.assertEqual(self.oled.oled.frames[-1][0][0], 'B')
    ...
    def test_bounds(self):
        ...
        for text,x,y in self.oled.oled.frames[-1]:
            self.assertLessEqual(len(text)*6,128); self.assertLessEqual(y,48)
    ...
    def test_api_success_get_clear(self):
        self.assertTrue(self.client.post('/api/v1.0/set-oled-text',json={'lines':['Temp: 21.5 Â°C']}).json['status'])
